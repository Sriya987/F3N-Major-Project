...
const GenerateSOAP: React.FC<GenerateSOAPProps> = ({ onNoteGenerated }) => {
  ...
  const handleMultimodalProcess = async () => {
    ...
    try {
      ...
      const rawSOAP = await geminiService.generateSOAPMultimodal(audioData, labData);
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="p-4 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 hover:border-blue-300 transition-colors">
                <label className="block text-sm font-bold text-slate-700 mb-3"><i className="fas fa-microphone mr-2 text-blue-500"></i>Encounter Audio</label>
...
//       alert(`Error processing clinical data: ${errorMsg}\n\nPlease check:\n1. Your GEMINI_API_KEY in .env.local is valid\n2. Audio/PDF files are properly formatted\n3. Check browser console for details`);
//     } finally {
//       setIsLoading(false);
//     }
//   };
...
//         assessment: refined.assessment || '',
//         plan: refined.plan || '',
//         rawDialogue: "[Audio Processed]",
//         rawLabData: "[PDF Processed]",
//         modelUsed: 'Hybrid (Multimodal Flash + Pro Refinement)',
//         timestamp: Date.now()
//       };
//       onNoteGenerated(finalNote);
//     } catch (error: any) {
...
//       const errorMsg = error?.message || error?.toString() || "Unknown error";
//       alert(`Error refining clinical text: ${errorMsg}\n\nPlease check:\n1. Your GEMINI_API_KEY in .env.local is valid\n2. The API key has proper permissions\n3. Check browser console for details`);
//     } finally {
//       setIsLoading(false);
//     }
//   };
