...
// OLLAMA SOAP GENERATION PROXY - Outside try block so it's always available
app.post('/api/ollama/generate', async (req, res) => {
  try {
    const { prompt, model = 'mistral:latest' } = req.body;
    console.log(`Ollama request with model: ${model}, prompt length: ${prompt?.length}`);
    ...
    if (!ollamaResponse.ok) {
      ...
      return res.status(500).json({ error: `Ollama failed: ${error}` });
    }
    ...
    res.status(500).json({ error: err.message || 'Ollama connection failed' });
  }
});
...
//   } catch (err) {
//     console.error("❌ Failed to connect to MongoDB Atlas:", err);
//     // Fix: Property 'exit' does not exist on type 'Process'. Cast to any to resolve the TS error.
